...
var configLoader = require('./loaders/node-loader');
...
function normalize(acc, val) {
	...
	if (typeof val === 'string') {
		var refPos = val.indexOf(acc.refLabel);
		if (refPos !== -1) {
			...
			if (command === 'disable') {
				...
				var result = val.replace(acc.refRegExp, function replaceMacro(val, path) {
					return get(path);
				}.bind(this));
				this.update(result);
			}
...
proto.build = function build() {
	configLoader(this._paths, {env: this._env, project: this._project}, function addConfig(config) {
