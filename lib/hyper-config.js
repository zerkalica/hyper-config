...
function merge(acc, val) {
	...
	if (Array.isArray(val)) {
		...
		if (typeof val === 'function' && val[acc.refLabel + 'definition']) {
			acc.defs.push(val[acc.refLabel + 'definition']);
			acc.set(path, val[acc.refLabel + 'name']);
		} else {
			acc.set(path, val);
		}
	}
...
function normalize(acc, val) {
	...
	if (this.key) {
		if (this.key.indexOf(acc.refLabel) === 0) {
...
proto.addConfig = function addConfig(config) {
	...
	var acc = traverse(config).reduce(merge, {set: set, refLabel: this._refLabel, defs: []});
	acc.defs.forEach(function addConfigFromDefs(config) {
		traverse(config).reduce(merge, {set: set, defs: []});
	}.bind(this));
